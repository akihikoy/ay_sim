...
double Gravity(-0.5);
bool Running(true), Shutdown(false), NoDisplay(false);
...
void TEnvironment::EDrawCallback()
{
  ...
  if(!NoDisplay && DrawCallback!=NULL)  DrawCallback();
...
bool TEnvironment::CollisionCallback(dBodyID &b1, dBodyID &b2, dContact *contact)
{
  ...
  for(int i(0); i<2; ++i)
  {
    ...
    {
      dBodyID bx= (b1==b_grippers_d[i] ? b2 : b1);
      for(int j(0); j<2; ++j)
        if(bx==b_grippers_a[j])
          return true;  // Ignore dummy gripper collision with actual gripper
      bool bx_is_ball(false);
      for(std::vector<TNCBody>::const_iterator itr(balls_.Body().begin()),itrend(balls_.Body().end());
          itr!=itrend; ++itr)
        {if(bx==itr->id())  {bx_is_ball= true; break;}}
      if(!bx_is_ball)  // If bx is a ball, we do not consider this as a gripper collision
...
static void SimStart()
{
  ...
  if(!NoDisplay)  dsSetViewpoint (xyz,hpr);
}
//-------------------------------------------------------------------------------------------
...
void Run(int argc, char **argv, const char *texture_path, int winx, int winy)
{
  ...
  {
    Running= false;  // Simulation is paused at the beginning.
    NoDisplay= true;
