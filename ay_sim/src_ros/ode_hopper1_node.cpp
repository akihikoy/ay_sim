...
*/
//-------------------------------------------------------------------------------------------
#include "ay_sim/ode_hopper1.h"
#include "ay_sim_msgs/ODEConfigHp1.h"
#include "ay_sim_msgs/ODESensorHp1.h"
#include "ay_sim_msgs/ODEControlHp1.h"
#include "ay_sim_msgs/ODEViz.h"
#include "ay_sim_msgs/ODEVizPrimitive.h"
#include "ay_sim_msgs/ODEGetConfigHp1.h"
#include "ay_sim_msgs/ODESetConfigHp1.h"
//-------------------------------------------------------------------------------------------
#include <ros/ros.h>
...
void OnDrawCallback()
{
  ...
  dMatrix3 R;
  dVector3 p;
  for(std::vector<ay_sim_msgs::ODEVizPrimitive>::const_iterator
      itr(VizObjs.begin()),itr_e(VizObjs.end()); itr!=itr_e; ++itr)
  {
    ...
    switch(itr->type)
    {
    case ay_sim_msgs::ODEVizPrimitive::LINE:
      p[0]= x[0]+itr->param[0];
      p[1]= x[1]+itr->param[1];
      p[2]= x[2]+itr->param[2];
      dsDrawLine(x, p);
      break;
    case ay_sim_msgs::ODEVizPrimitive::SPHERE:
      dRfromQ(R,q);
      dsDrawSphere(x, R, /*rad=*/itr->param[0]);
      break;
    case ay_sim_msgs::ODEVizPrimitive::CYLINDER:
      dRfromQ(R,q);
      dsDrawCylinder(x, R, /*len=*/itr->param[1], /*rad=*/itr->param[0]);
      break;
    case ay_sim_msgs::ODEVizPrimitive::CUBE:
      dRfromQ(R,q);
      sides[0]= itr->param[0];
      sides[1]= itr->param[1];
      sides[2]= itr->param[2];
...
void ControlCallback(const ay_sim_msgs::ODEControlHp1 &msg)
{
...
void ODEVizCallback(const ay_sim_msgs::ODEViz &msg)
{
  VizObjs= msg.objects;
}
//-------------------------------------------------------------------------------------------

bool GetConfig(ay_sim_msgs::ODEGetConfigHp1::Request &req, ay_sim_msgs::ODEGetConfigHp1::Response &res)
{
  using namespace ode_x;
  ay_sim_msgs::ODEConfigHp1 &msg(res.config);
...
bool ResetSim2(ay_sim_msgs::ODESetConfigHp1::Request &req, ay_sim_msgs::ODESetConfigHp1::Response &res)
{
  using namespace ode_x;
  std::cerr<<"Resetting simulator..."<<std::endl;
  const ay_sim_msgs::ODEConfigHp1 &msg(req.config);
...
int main(int argc, char**argv)
{
  ...
  ros::Publisher sensors_pub= node.advertise<ay_sim_msgs::ODESensorHp1>("sensors", 1);
